...
правило `@font-face` только для определённого ряда устройств и размеров экрана с 
помощью медиазапросов. К сожалению, такой подход не работает в некоторых 
браузерах, включая все версии Internet Explorer и FireFox старше 10 версии. В 
этой статье описано решение этой проблемы, которое позволяет достичь баланса 
...
просматривая вебсайты на смартфоне: блок пустого пространства там, где должен 
...
Чтобы упростить объяснение предложенного решения, создадим два файла: index.html 
и style.css. Первый — это образец страницы с текстом, к которому применён 
подключённый шрифт, второй содержит стили для этой страницы. 
...
подключённый шрифт, используя [ошибкоустойчивый @font-face синтаксис от 
Fontspring][4]. Затем мы просто применяем новый шрифт для всех абзацев, заменяя 
прописанный ранее стиль. 

## Совместимость с браузерами

На первый взгляд это решение кажется действительно отличным, так как позволяет 
...
старше, а также [Firefox 10 и старше][5]. Вам может показаться такое поведение 
странным и ошибочным, однако на самом деле оно продиктовано [спецификацией 
CSS2.1][6], согласно которой @-правила внутри медиазапросов не работают в CSS2.1.
...
поддержку][7] в версии после 10. Таким образом, по большому счёту проблема 
касается только Internet Explorer. Также следует помнить что Internet Explorer 8 
и старше не понимают медиазапросы, так что даже если новые версии и будут 
обновлены, проблема со старыми никуда не денется. 
...
не узнал как её решить. Так как найти решение я не смог, осталось придумать его 
самостоятельно. 
...
что он наш код с условным комментарием проигнорирует. С нашей проблемой ещё не 
покончено. 
...
Стоит задаться вопросом как это решение будет работать для вебсайта с отзывчивым 
...
ничего нельзя поделать. Мне кажется это очень хороший результат. 
...
по настольных компьютерах следующая:
...
Так как мы оставили правило `@font-face` внутри медиазапроса, наш вебсайт 
...
диапазон входят все браузеры кроме Internet Explorer. Тоесть, мы получаем 
